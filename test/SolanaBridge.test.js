...
const { mockCreateSolanaToken, verifyBridgeCompatibility, simulateBridgeDeposit } = require("./helpers/multichain");
...
describe("Solana Bridge Integration", function () {
  ...
  describe("Wormhole Bridge Compatibility", function () {
    ...
    it("Should handle Wormhole token locking", async function () {
      ...
      const result = await simulateBridgeDeposit(tokenL1, WORMHOLE_ETHEREUM, user, bridgeAmount);
  ...
  describe("Bridge Security", function () {
    it("Should prevent bridging when paused", async function () {
      ...
      await expect(tokenL1.connect(user).approve(WORMHOLE_ETHEREUM, amount)).to.not.be.reverted;

      // But transfer should fail
      await expect(tokenL1.connect(user).transfer(WORMHOLE_ETHEREUM, amount)).to.be.revertedWithCustomError(
        tokenL1,
        "EnforcedPause"
      );
    });
  ...
  describe("Multi-Bridge Scenario", function () {
    ...
    it("Should track bridge balances separately", async function () {
      ...
      expect(await tokenL1.balanceOf(user.address)).to.equal(ethers.parseEther("1000") - amount1 - amount2);
    });
  });
});
